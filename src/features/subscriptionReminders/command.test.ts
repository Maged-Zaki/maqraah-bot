...
test('/reminders subscribe adds the category subscription privately', { concurrency: false }, async () => {
	...
	await withCommandMocks(
		{
			subscribeMemberToCategory: async () => ({ changed: true, role: { id: 'role-muhammed-way', name: 'صيام محمد ﷺ' } }),
		},
		async () => {
	...
	assert.deepEqual(replies, [{ content: 'You are subscribed to صيام محمد ﷺ.', flags: MessageFlags.Ephemeral }]);
});
...
test('/reminders unsubscribe removes the category subscription privately', { concurrency: false }, async () => {
	...
	await withCommandMocks(
		{
			unsubscribeMemberFromCategory: async () => ({ changed: true, role: { id: 'role-muhammed-way', name: 'صيام محمد ﷺ' } }),
		},
		async () => {
	...
	assert.deepEqual(replies, [{ content: 'You are unsubscribed from صيام محمد ﷺ.', flags: MessageFlags.Ephemeral }]);
});

test('/reminders list shows the caller subscriptions privately', { concurrency: false }, async () => {
	const replies: any[] = [];
	const role = { id: 'role-muhammed-way', name: 'صيام محمد ﷺ' };
	...
	assert.match(replies[0].content, /Subscribed: صيام محمد ﷺ/);
